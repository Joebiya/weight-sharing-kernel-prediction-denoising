...
class SMAPELoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        loss = torch.mean(
            ...
            / (denominator.abs() + targets.abs() + self.eps)
        )
        return loss
            ...
            if dataset.use_val:
                ...
                for i in range(output.shape[0]):
                    if np.sum(target[i]) == 0.0:
                        continue
                    SSIM, PSNR = ComputeMetrics(
                        target[i].transpose((1, 2, 0)), output[i].transpose((1, 2, 0))
                    )
                    SSIMs.append(SSIM)
                    PSNRs.append(PSNR)
...
if __name__ == "__main__":
    ...
    dataloader_val = torch.utils.data.DataLoader(
        dataset_val, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=True
    )
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % epoch_test == 0:
            ...
            if epoch > epochs * 0.8:
                _SSIM_test, _PSNR_test = Inference(
                    model, device, dataset_test, dataloader_test, test_saving_root
                )
            ...
            torch.save(
                ...
                os.path.join(model_saving_path, "model.pt"),
            )
